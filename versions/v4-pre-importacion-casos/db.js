...
const STORAGE_KEYS = {
  CASES: 'lims_cases_v4legacy',
  PATHOLOGIES: 'lims_pathologies_v4legacy',
};
...
/** @returns {boolean} true si se guardó. Si el navegador rechaza el write (cuota llena), no pierde el error en silencio. */
function writeStore(key, value) {
  try {
    ...
    Utils.toast('No se pudo guardar: el almacenamiento del navegador está lleno. Exporta y elimina casos antiguos para liberar espacio.', 'error');
    return false;
  }
}
...
const DB = {
  ...
  pathologies: {
    ...
    create(data) {
      ...
      all.push(record);
      return writeStore(STORAGE_KEYS.PATHOLOGIES, all) ? record : null;
    },
    update(id, data) {
      const all = this.getAll();
      ...
      return writeStore(STORAGE_KEYS.PATHOLOGIES, all) ? all[idx] : null;
    },
    remove(id) {
      const all = this.getAll().filter((p) => p.id !== id);
      return writeStore(STORAGE_KEYS.PATHOLOGIES, all);
    },
  },
  ...
  cases: {
    ...
    create(data) {
      ...
      const record = { id: Utils.generateId(), ...data };
      all.push(record);
      return writeStore(STORAGE_KEYS.CASES, all) ? record : null;
    },
    update(id, data) {
      const all = this.getAll();
      ...
      if (idx === -1) return null;
      all[idx] = { ...all[idx], ...data, id };
      return writeStore(STORAGE_KEYS.CASES, all) ? all[idx] : null;
    },
    remove(id) {
      const all = this.getAll().filter((c) => c.id !== id);
      return writeStore(STORAGE_KEYS.CASES, all);
    },
  },
};
