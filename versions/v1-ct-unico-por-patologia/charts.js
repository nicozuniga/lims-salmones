...
const Charts = {
  ...
  renderPositivesByPathology(cases) {
    ...
    cases.forEach((c) => (c.pathologies || []).forEach((p) => {
      ...
      if (p.result === 'POSITIVO') counts[p.name] += 1;
    }));
    const sorted = Object.entries(counts).sort((a, b) => b[1] - a[1]).slice(0, 8);
  ...
  renderPositivePercentage(cases) {
    ...
    let gray = 0;
    cases.forEach((c) => (c.pathologies || []).forEach((p) => {
      if (p.result === 'POSITIVO') pos += 1;
      else if (p.result === 'NEGATIVO') neg += 1;
      else if (p.result === 'OBSERVAR') gray += 1;
    }));
  ...
  renderCtDistribution(cases) {
    const cts = [];
    cases.forEach((c) => (c.pathologies || []).forEach((p) => {
      if (p.ctObtained !== '' && p.ctObtained !== null && p.ctObtained !== undefined) cts.push(Number(p.ctObtained));
    }));
    const bucketSize = 2;
    const buckets = {};
    cts.forEach((ct) => {
