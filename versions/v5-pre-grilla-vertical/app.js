...
// ============================================================
// Importar casos desde CSV/Excel (mismas columnas que "Exportar caso")
// ============================================================
...
el('btn-import-cases').addEventListener('click', () => importFileInput.click());

importFileInput.addEventListener('change', async (e) => {
  const file = e.target.files[0];
  importFileInput.value = ''; // permite volver a elegir el mismo archivo si hace falta reintentar
  if (!file) return;

  let rows;
  try {
    rows = await parseCaseFile(file);
  } catch (err) {
    console.error(err);
    Utils.toast('No se pudo leer el archivo. Verifica que sea un CSV o Excel válido.', 'error');
    return;
  }

  const columnError = Utils.validateCaseImportColumns(rows);
  if (columnError) return Utils.toast(columnError, 'error');

  const parsedCases = Utils.buildCasesFromImportRows(rows, DB.pathologies.getAll());
  if (!parsedCases.length) return Utils.toast('No se encontraron casos válidos en el archivo.', 'error');

  const existing = DB.cases.getAll();
  const updateCount = parsedCases.filter((c) => existing.some((e) => e.caseNumber === c.caseNumber)).length;
  const createCount = parsedCases.length - updateCount;
  const ok = await Utils.confirmDialog(
    `Se importarán ${parsedCases.length} caso(s): ${createCount} nuevo(s) y ${updateCount} actualizado(s) (mismo N° de caso ya existente). ¿Continuar?`
  );
  if (!ok) return;
  ...
  parsedCases.forEach((c) => {
    const match = existing.find((e) => e.caseNumber === c.caseNumber);
    const result = match ? DB.cases.update(match.id, c) : DB.cases.create(c);
    if (!result) failed += 1;
  ...
  refreshCasesView();
  if (failed) Utils.toast(`Importación con errores: ${failed} caso(s) no se pudieron guardar.`, 'error');
  else Utils.toast(`Importación completa: ${created} caso(s) nuevos, ${updated} actualizados.`, 'success');
});
