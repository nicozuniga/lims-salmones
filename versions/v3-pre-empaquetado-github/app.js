...
function renderCasesTable() {
  ...
  casesTbody.innerHTML = cases.map(buildCaseRowHtml).join('');
}
...
function runExplorer() {
  ...
  explorerTbody.innerHTML = results.map(buildCaseRowHtml).join('');
  explorerEmpty.hidden = results.length > 0;
  explorerEmpty.textContent = 'No hay casos que coincidan con la búsqueda.';
}

explorerPathology.addEventListener('change', runExplorer);
explorerClient.addEventListener('change', runExplorer);
